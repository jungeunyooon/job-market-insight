...
class TestLlmSummarize:
    ...
    @patch("nlp.llm_summarizer.urlopen")
    def test_returns_summary_on_success(self, mock_urlopen):
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps(
            {"response": "Redis 캐싱 아키텍처를 도입하여 성능을 3배 개선했습니다."}
        ).encode("utf-8")
    ...
    def test_truncates_long_content(self, mock_urlopen):
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps({"response": "요약"}).encode("utf-8")
        mock_response.__enter__ = MagicMock(return_value=mock_response)
        mock_response.__exit__ = MagicMock(return_value=False)
        mock_urlopen.return_value = mock_response
