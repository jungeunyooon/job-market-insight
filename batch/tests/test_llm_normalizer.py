...
class TestLlmNormalizerWithMock:
    ...
    @patch("nlp.llm_normalizer.urlopen")
    def test_normalizes_requirements(self, mock_urlopen):
        mock_urlopen.return_value = self._mock_urlopen(MOCK_NORMALIZED_RESPONSE)
    ...
    def test_handles_codeblock_response(self, mock_urlopen):
        ...
        mock_urlopen.return_value = self._mock_urlopen(codeblock)
    ...
    def test_handles_invalid_json(self, mock_urlopen):
        mock_urlopen.return_value = self._mock_urlopen("유효하지 않은 응답입니다")
    ...
    def test_only_requirements_no_preferred(self, mock_urlopen):
        ...
        mock_urlopen.return_value = self._mock_urlopen(single)
