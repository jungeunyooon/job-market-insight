...
Uses LLM to extract contextual technical keywords from blog content —
...
def llm_extract_blog_keywords(
    ...
    """LLM API를 호출하여 블로그 글에서 심층 기술 키워드를 추출한다.

    Returns:
        [{"keyword": str, "context": str}] 리스트. LLM 미사용/오류 시 빈 리스트.
    """
    ...
    try:
        ...
        logger.warning("LLM 블로그 키워드 추출 실패: %s", exc)
        return []
    except Exception as exc:
        logger.warning("LLM 블로그 키워드 추출 예외: %s", exc)
        return []
